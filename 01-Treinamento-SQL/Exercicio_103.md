...
--Opção 1:
select 
    count(distinct c.id_cliente) as "Quantidade de contas ativadas"
from 
    t_cliente c
join 
    t_venda v on c.id_cliente = v.id_cliente 
where 
    c.fl_status_conta = 'A' and v.fl_status_venda = 'A';

-- Opção 2:
select 
    count(c.id_cliente) as "Quantidade de contas ativadas"
from 
    t_cliente c 
where 
    id_cliente in (
